...
const char * assymetricKeyOperation = "asymmetric_key";

gboolean asymmetric_key_spiner_close_cb(GtkDialog * spinnerDialog, gpointer user_data)
{
    return TRUE;
}

static void asymmetric_key_ok(struct asymmetric_key_data * data, const char * operationString)
{
    printf("asymmetric_key_ok\n");
...
static void asymmetric_key_error(struct asymmetric_key_data * data, int errorCode, const char * operationString)
{
    printf("asymmetric_key_error\n");
...
static void _asymmetric_key_progress_finish(GObject * spinnerDialog, GAsyncResult * res, gpointer user_data)
{
    const char * asymmetric_key_check = "Sprawdzanie kluczy";
    const char * asymmetric_key_rsa_gen = "Generowanie kluczy RSA";
    const char * asymmetric_key_on_param_gen = "Generowanie kluczy na parametrach";
    const char * operationString = NULL;
    int mode = 0;

    struct asymmetric_key_data * data = (struct asymmetric_key_data*)user_data;

    gtk_widget_set_visible(GTK_WIDGET(spinnerDialog), FALSE);

    int iResult = g_task_propagate_int(G_TASK(res), NULL);

    printf("_asymmetric_key_progress_finish %d\n", iResult);

    mode = (data->flags & ASSYMETRIC_KEY_MODE_MASK);
    switch(mode)
    {
    case ASYMMETRIC_KEY_RSA_GEN:
        operationString = asymmetric_key_rsa_gen;
        break;
    case ASSYMETRIC_KEY_ON_PARAM_GEN:
        operationString = asymmetric_key_on_param_gen;
        break;
    case ASSYMETRIC_KEY_CHECK:
        operationString = asymmetric_key_check;
        break;
    }

    if(iResult == 1)
        asymmetric_key_ok(data, operationString);
    else
        asymmetric_key_error(data, -iResult, operationString);

    gtk_window_present(GTK_WINDOW(data->alertDialog));
}

static int _asymmetric_key_progress_func(GObject * spinnerDialog, GAsyncResult * res, gpointer user_data, GCancellable * cancellable)
{
    struct asymmetric_key_data * data = (struct asymmetric_key_data*)user_data;

    printf("_asymmetric_key_progress_func\n");
    ...
    pthread_t thread;
    struct async_key_data * async_data = NULL;
    ...
    async_data = async_key_data_new();
    if(async_data == NULL)
        goto error;

    async_key_data_provide(async_data, data->passphrase, data->keyPubPath, data->keyPrivPath, data->paramPath, data->keySize,
    ...
    {
    case ASYMMETRIC_KEY_RSA_GEN:
        pthread_create(&thread, NULL, async_generate_rsa_key, async_data);
        break;
    case ASSYMETRIC_KEY_ON_PARAM_GEN:
        pthread_create(&thread, NULL, async_generate_on_param_key, async_data);
        break;
    case ASSYMETRIC_KEY_CHECK:
        pthread_create(&thread, NULL, async_check, async_data);
        break;
    }
...
//end:
    ...
    if(realResult == 1)
        jobs_log_async_write(assymetricKeyOperation, 0);
    else
        jobs_log_async_write(assymetricKeyOperation, 1);
    ...
    if(async_data != NULL)
        async_key_data_delete(async_data);

    return realResult;
}

static void _asymmetric_key_progress_thread(GTask * task, void * spinnerDialog, gpointer user_data, GCancellable * cancellable)
{
    int iResult = 1;

    printf("_asymmetric_key_progress_thread\n");

    iResult = _asymmetric_key_progress_func(spinnerDialog, G_ASYNC_RESULT(task), user_data, cancellable);

    g_task_return_int(task, iResult);
}

static void _asymmetric_key_progress_async(GtkWidget * spinnerDialog, GCancellable * cancellable, GAsyncReadyCallback arc, gpointer user_data)
{
    GTask * task = g_task_new(spinnerDialog, cancellable, arc, user_data);

    printf("_asymmetric_key_progress_async\n");

    g_task_set_task_data(task, user_data, (GDestroyNotify)asymmetric_key_data_clear);
    g_task_set_return_on_cancel(task, TRUE);
    g_task_run_in_thread(task, _asymmetric_key_progress_thread);

    g_object_unref(task);
}

void asymmetric_key_progress_init(gpointer user_data)
{
    const char * operationString = NULL;
    static const char * keyCheckKey = "Sprawdzanie kluczy...";
    static const char * keyRsaGenKey = "Generowanie kluczy RSA...";
    static const char * keyParamGenKey = "Generowanie kluczy na parametrach...";
    int mode = 0;

    struct asymmetric_key_data * data = (struct asymmetric_key_data*)user_data;

    printf("asymmetric_key_progress_init\n");

    mode = (data->flags & ASSYMETRIC_KEY_MODE_MASK);
    switch(mode)
    {
    case ASYMMETRIC_KEY_RSA_GEN:
        operationString = keyCheckKey;
        break;
    case ASSYMETRIC_KEY_ON_PARAM_GEN:
        operationString = keyRsaGenKey;
        break;
    case ASSYMETRIC_KEY_CHECK:
        operationString = keyParamGenKey;
        break;
    }
    ...
    _asymmetric_key_progress_async(data->spinnerDialog, NULL, _asymmetric_key_progress_finish, user_data);
}
...
void asymmetric_key_password_cb(GtkDialog * passwordDialog, gint response_id, gpointer user_data)
{
    ...
    printf("asymmetric_key_password_cb\n");
    ...
    asymmetric_key_data_provide_password(data, password);
    ...
    asymmetric_key_progress_init(user_data);
}
...
gboolean asymmetric_key_password_close_cb(GtkDialog * passwordDialog, gpointer user_data)
{
    ...
    printf("asymmetric_key_password_close_cb\n");

    gtk_widget_set_visible(GTK_WIDGET(passwordDialog), FALSE);
    return TRUE;
}

static void asymmetric_key_proceed(gpointer user_data, unsigned char mode)
{
    struct asymmetric_key_wrapper * wrapper = (struct asymmetric_key_wrapper*)user_data;
    struct asymmetric_key_data * data = wrapper->data;
    ...
    printf("asymmetric_key_proceed\n");

    asymmetric_key_data_provide_flags(data, mode | IN_USE);
    asymmetric_key_wrapper_provide_data(wrapper, data);
    iResult = asymmetric_key_data_verify(data, mode);
    cipher = get_symmetric_algorithm(CIPHER_FULL_ADDRESS(data->privAlgorithm, data->privKeySize, data->privBlockMode));
    if(cipher == NULL || iResult != ASSYMETRIC_KEY_OK)
    {
        asymmetric_key_data_clear(data);
        return;
    }

    //if(mode == ASYMMETRIC_KEY_RSA_GEN || mode == ASSYMETRIC_KEY_ON_PARAM_GEN)
        gtk_window_present(GTK_WINDOW(data->passwordDialog));
    //else
        //asymmetric_key_progress_init(data);
}

void asymmetric_key_rsa_gen(GtkButton * rsaGenButton, gpointer user_data)
{
    printf("asymmetric_key_rsa_gen\n");
    asymmetric_key_proceed(user_data, ASYMMETRIC_KEY_RSA_GEN);
}

void asymmetric_key_on_param_gen(GtkButton * keyOnParamGenButton, gpointer user_data)
{
    printf("asymmetric_key_on_param_gen\n");
    asymmetric_key_proceed(user_data, ASSYMETRIC_KEY_ON_PARAM_GEN);
}

void asymmetric_key_check(GtkButton * keyCheckButton, gpointer user_data)
{
    printf("asymmetric_key_check\n");
    asymmetric_key_proceed(user_data, ASSYMETRIC_KEY_CHECK);
}

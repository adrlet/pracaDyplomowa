...
struct asymmetric_seal_wrapper * asymmetric_seal_wrapper_new()
{
    ...
    wrapper = (struct asymmetric_seal_wrapper*)malloc(sizeof(struct asymmetric_seal_wrapper));
    if(wrapper == NULL)
        return wrapper;
                                             ...
                                             GtkWidget * privAlgorithmDropDown, GtkWidget * privBlockModeDropDown, GtkWidget * privKeySizeDropDown)
{
    ...
    wrapper->ivPathEntry = ivPathEntry;
...
void asymmetric_seal_wrapper_provide_data(struct asymmetric_seal_wrapper * wrapper, struct asymmetric_seal_data * data)
{
    ...
    if(wrapper->ivPathEntry != NULL)
    {
        entryBuffer = gtk_entry_get_buffer(GTK_ENTRY(wrapper->ivPathEntry));
        path = gtk_entry_buffer_get_text(entryBuffer);
        if(path != NULL && path[0] != 0)
        {
            data->ivPath = (char*)malloc(sizeof(char)*(strlen(path)+1));
            strcpy(data->ivPath, path);
            printf("%s\n", data->ivPath);
        }
    }
    ...
    if(wrapper->base64Check != NULL)
        if(gtk_check_button_get_active(GTK_CHECK_BUTTON(wrapper->base64Check)) == TRUE)
            data->flags = data->flags | SEAL_BASE64;
